...
BOOL RopeEncoderCalibrationDlg::OnInitDialog()
{
	...
	// IDC_COMBO2 has CBS_SORT, so index 0 is not stable (it sorts "粗标"
	// before "拉线编码器" on this system). Select the workflow by exact text.
	const int ropeCalibrationIndex = m_CalibTypeCombo.FindStringExact(-1, _T("拉线编码器"));
	m_CalibTypeCombo.SetCurSel(ropeCalibrationIndex >= 0 ? ropeCalibrationIndex : 0);
	...
	if (!kOfflinePreviewMode)
	{
		...
		if (!gQKMLinkReady)
		{
			// Do not block OnInitDialog with a modal message before the main window
			// becomes usable.  A workstation without the registered 32-bit
			// QKMLink component must still be able to open the solution, inspect the
			// HE3_GY seven-axis/60-pose configuration and run offline tests.  Machine
			// controls remain disabled until communication is available.
			CString unavailableTitle;
			GetWindowText(unavailableTitle);
			unavailableTitle += _T(" [QKMLink未就绪]");
			SetWindowText(unavailableTitle);
			OutputDebugString(QKMLinkLastInitErrorText());
			GetDlgItem(IDC_BUTTON1)->EnableWindow(FALSE);
...
void RopeEncoderCalibrationDlg::OnBnClickedButton2()
{
	...
	if (!std::isfinite(m_ThisRbt.RopeEncoderCountsPerMillimeter)
		|| m_ThisRbt.RopeEncoderCountsPerMillimeter <= 0.0)
	{
		MachineTrace("START_BLOCKED", "invalid rope encoder counts-per-millimetre conversion");
		MessageBox(_T("拉线编码器换算系数无效，请检查 HE3_GY 配置第16项。"), _T("配置不匹配"), MB_OK | MB_ICONERROR);
		return;
	}
	if (kOfflinePreviewMode)
	{
...
void RopeEncoderCalibrationDlg::OnBnClickedButton7()
{
	...
	// In controlled HE3 machine-test mode, never turn missing samples into
	// numeric zeros.  Calibration can otherwise appear to run while fitting a
	// mixture of real and absent measurements.
	if (kMachineTestMode && m_RobotAxis == 7)
	{
		if (m_TotalNum != 60 || m_LocIdx != m_TotalNum || m_RobotHere == nullptr)
		{
			MachineTrace("CALIBRATION_BLOCKED", "joint feedback sample count is not exactly 60");
			MessageBox(_T("关节反馈数据不完整：必须成功收到60组七轴实际角度，算法才会启动。"),
				_T("数据不完整"), MB_OK | MB_ICONERROR);
			return;
		}
		for (int sample = 0; sample < m_TotalNum; ++sample)
		{
			CString jointText = m_RobotHere[sample];
			jointText.Trim();
			CString ropeText = m_List.GetItemText(sample, 2);
			ropeText.Trim();
			if (jointText.IsEmpty() || ropeText.IsEmpty())
			{
				MachineTrace("CALIBRATION_BLOCKED", "empty joint or rope sample at index=" + std::to_string(sample + 1));
				CString message;
				message.Format(_T("第%d点缺少关节反馈或拉线读数，已禁止计算。"), sample + 1);
				MessageBox(message, _T("数据不完整"), MB_OK | MB_ICONERROR);
				return;
			}
		}
	}
	...
	for (int ii = 0; ii < m_TotalNum; ii++) {
		...
		EncoderValueStr.Trim();
		TCHAR* parseEnd = nullptr;
		const double valueMm = _tcstod(EncoderValueStr.GetString(), &parseEnd);
		while (parseEnd != nullptr && _istspace(*parseEnd))
			++parseEnd;
		if (EncoderValueStr.IsEmpty() || parseEnd == EncoderValueStr.GetString()
			|| (parseEnd != nullptr && *parseEnd != _T('\0')) || !std::isfinite(valueMm))
		{
			MachineTrace("CALIBRATION_BLOCKED", "invalid rope numeric value at index=" + std::to_string(ii + 1));
			CString message;
			message.Format(_T("第%d点拉线读数不是有效数字，已禁止计算。"), ii + 1);
			MessageBox(message, _T("数据无效"), MB_OK | MB_ICONERROR);
			return;
		}
		EncoderValue[ii] = valueMm;
	}
	...
	for (int sample = 0; sample < m_TotalNum; ++sample)
	{
		const double effectiveLength = EncoderValue(sample) + m_ThisRbt.RopeLengthOffset;
		if (!std::isfinite(effectiveLength) || effectiveLength <= 0.0)
		{
			MachineTrace("CALIBRATION_BLOCKED", "non-positive effective rope length at index=" + std::to_string(sample + 1));
			CString message;
			message.Format(_T("第%d点加上治具偏置后的拉线长度不是正数，已禁止计算。"), sample + 1);
			MessageBox(message, _T("数据无效"), MB_OK | MB_ICONERROR);
			return;
		}
	}
	...
	else 
	{
		...
		if (m_RobotAxis == 6 || m_RobotAxis == 7)
		{
			...
			if (m_RobotAxis == 7)
			{
				// HE3 self-centring rope fixture: the 49 mm item is the constant
				// J7-centre-to-rope radius, not a direction-fixed TCP.  The measured
				// centre is selected before the final d7 translation below, therefore
				// d7 itself is not observable from this measurement and must be frozen.
				// q1/q6/q7 also remain frozen for the current single-anchor data set.
				const int dAxes[] = { 1, 2, 3, 4 };
			...
			const bool he3SelfCentringFixture = (m_RobotType == 3 && m_RobotAxis == 7);
			if (he3SelfCentringFixture)
			{
				// GetJ2JTrans applies d7 before returning the final frame.  A local
				// offset of -d7 selects the centre immediately before that translation.
				// The rotating U-bracket preserves only its radius, so that radius is
				// added to the rope length instead of being used as a fixed XYZ vector.
				options.toolOffset[2] = -mdh[m_RobotAxis + m_RobotAxis - 1];
				options.ropeLengthOffset = m_ThisRbt.RopeLengthOffset + m_ThisRbt.ToolOffset;
			}
			else
			{
			...
			if (report.is_open())
			{
				...
				report << "Measurement model = "
					<< (he3SelfCentringFixture ? "HE3 J7 centre + self-centring radius" : "Rigid tool point") << "\n";
				report << "Kinematic measurement point local Z(m) = " << options.toolOffset[2] << "\n";
				report << "Fixture radius from config(m) = " << m_ThisRbt.ToolOffset << "\n";
				report << "Rope length offset(m) = " << options.ropeLengthOffset << "\n";
				report << "Rope encoder counts/mm = " << m_ThisRbt.RopeEncoderCountsPerMillimeter << "\n";
...
void RopeEncoderCalibrationDlg::OnCbnSelchangeCombo1()
{
	CString selectitem;
	CString str;
	const int selectedIndex = m_RbtTypeCombo.GetCurSel();
	if (selectedIndex == CB_ERR)
	{
		MachineTrace("CONFIG_REJECTED", "robot combo has no valid selection");
		GetDlgItem(IDC_BUTTON1)->EnableWindow(FALSE);
		return;
	}
	m_RbtTypeCombo.GetLBText(selectedIndex, selectitem);
	string rbtType = CT2A(selectitem.GetBuffer(0));
	const string configPath = _rbtConfigPath + '/' + rbtType + _configExtension;
	if (rbtType.empty() || !ReadConfigFile(configPath, m_ThisRbt))
	{
		m_TotalNum = 0;
		m_RobotAxis = 0;
		m_RobotType = -1;
		delete[] m_RobotHere;
		m_RobotHere = nullptr;
		m_ThisRbtDH.resize(0, 0);
		m_List.DeleteAllItems();
		GetDlgItem(IDC_BUTTON1)->EnableWindow(FALSE);
		MachineTrace("CONFIG_REJECTED", "missing or invalid robot configuration: " + configPath);
		CString errorText;
		errorText.Format(_T("机器人配置缺失或格式错误，程序已停止初始化，未访问无效数组。\n\n文件：%S"), configPath.c_str());
		MessageBox(errorText, _T("配置错误"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	// RobotAxis and all seven configuration arrays were validated by
	// ReadConfigFile before this matrix was allocated.  The legacy code reached
	// column -1 when a configuration file was missing, causing 0xC0000005.
	...
	if (File_Exist(m_FilePath))
	{
		...
		if (joint.rows() != m_RobotAxis || joint.cols() != m_TotalNum)
		{
			m_List.DeleteAllItems();
			GetDlgItem(IDC_BUTTON1)->EnableWindow(FALSE);
			MachineTrace("CONFIG_REJECTED", "robot location file has missing/non-numeric values: " + m_FilePath);
			CString errorText;
			errorText.Format(_T("机器人点位文件内容不足或包含非数字，程序已停止初始化。\n\n应为 %d 轴 × %d 点。\n文件：%S"),
				m_RobotAxis, m_TotalNum, m_FilePath.c_str());
			MessageBox(errorText, _T("点位文件错误"), MB_OK | MB_ICONERROR);
			return;
		}
	...
	msg += (" Rope encoder counts/mm = ") + to_string(m_ThisRbt.RopeEncoderCountsPerMillimeter) + "\n";
	...
	MachineTrace("CONFIG", "robot=" + rbtType + "; type=" + std::to_string(m_RobotType)
		...
		+ "; ropeCountsPerMm=" + std::to_string(m_ThisRbt.RopeEncoderCountsPerMillimeter)
...
{
	CString listResult;
	// Convert the raw counter using the audited HE3_GY configuration value.
	// The default remains the legacy calibrated value 284.94 counts/mm.
	const double countsPerMillimetre = m_ThisRbt.RopeEncoderCountsPerMillimeter;
	if (!std::isfinite(countsPerMillimetre) || countsPerMillimetre <= 0.0)
	{
		MachineTrace("POSE_MEASUREMENT_REJECTED", "invalid rope encoder conversion scale");
		return -1;
	}
	double EncoderValue = (lParam - m_EncoderHomeValue) / countsPerMillimetre;
	MachineTrace("POSE_MEASUREMENT", "index=" + std::to_string(static_cast<int>(wParam) + 1)
		...
		+ "; countsPerMm=" + std::to_string(countsPerMillimetre)
...
UINT RopeEncoderCalibrationDlg::ZeroThread(LPVOID pParam) {
	try
	{
		...
		CString AxisValue;
		// The legacy tool allocated exactly six joints here, but ZeroThread also
		// loops to m_RobotAxis.  HE3_GY has seven joints, so the old array wrote
		// past its end before the 60-pose acquisition even started.
		std::vector<double> Axis(static_cast<size_t>(m_RobotAxis), 0.0);
		char spilt = ' ';
		int sIdx = 0;
		int eIdx = 0;
		int errCode = 0;
		...
		if (m_RobotType == Cobot) 
		{
			...
			Axis[4] = 0;
			Axis[5] = 0;
			AxisValue = TurnDoubleToLoc(Axis.data());
		...
		// m_MoveEvent and m_WhereEvent are application-lifetime synchronization
		// handles.  The 60-pose thread reuses both immediately after homing, so
		// they must not be nulled here.  Their owner closes them during shutdown.
	}
	catch (const std::exception&)
	{
