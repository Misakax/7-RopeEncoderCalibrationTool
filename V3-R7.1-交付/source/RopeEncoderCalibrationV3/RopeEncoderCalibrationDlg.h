...
public:
	...
	void AppendR6Monitor(const CString& text, bool mirrorToFile = true);
	void ShowCurrentMdh(const CString& operation);
	void ShowR65ModelContract(const CString& operation);
	bool ReadControllerArray(const CString& idn, int count, std::vector<double>& values, const CString& label);
	bool ReloadControllerModelAsInput(const CString& reason, bool updateInput);
	bool WriteControllerValueVerified(const CString& idn, int arrayIndex, double value,
		double tolerance, CString& failure);
	bool ValidateR65Candidate(const std::vector<double>& dMm,
		const std::vector<double>& legacyAlphaDeg,
		const std::vector<double>& zeroCounts, CString& failure) const;
	void ShowLastCalibrationSummary();
	bool PrepareR71PublicInputBeforeStart();
	...
	afx_msg void OnBnClickedR6Feedback();
	afx_msg void OnBnClickedR6Restore();
	...
	CEdit m_ErrorAfter;
	CEdit m_RopeBias;
	CButton m_R6FeedbackBtn;
	CButton m_R6RestoreBtn;
	CEdit m_R6Monitor;
