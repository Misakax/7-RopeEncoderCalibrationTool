...
struct State
{
    ...
    Vector3d anchor;
    double ropeBias = 0.0; // L_measured = |P-anchor| + ropeBias
    ...
    double maeBeforeZeroBias = std::numeric_limits<double>::infinity();
    double maxAbsBeforeZeroBias = std::numeric_limits<double>::infinity();
    int anchorRank = 0;
};
...
int EvaluateResiduals(const VectorXd& measured, State& state)
{
    const int sampleCount = static_cast<int>(state.points.cols());
    state.residual.resize(sampleCount);
    double rawSumAbs = 0.0;
    double rawMaxAbs = 0.0;
    double correctedSumAbs = 0.0;
    double correctedMaxAbs = 0.0;
    for (int s = 0; s < sampleCount; ++s)
    {
        const double distance = (state.points.col(s) - state.anchor).norm();
        if (!IsFinite(distance) || distance <= kTinyLength)
            return CALV3_ERR_INVALID_LENGTH;
        const double rawResidual = measured(s) - distance;
        const double correctedResidual = rawResidual - state.ropeBias;
        if (!IsFinite(rawResidual) || !IsFinite(correctedResidual))
            return CALV3_ERR_NONFINITE;
        state.residual(s) = correctedResidual;
        rawSumAbs += std::abs(rawResidual);
        rawMaxAbs = std::max(rawMaxAbs, std::abs(rawResidual));
        correctedSumAbs += std::abs(correctedResidual);
        correctedMaxAbs = std::max(correctedMaxAbs, std::abs(correctedResidual));
    }
    state.maeBeforeZeroBias = rawSumAbs / static_cast<double>(sampleCount);
    state.maxAbsBeforeZeroBias = rawMaxAbs;
    state.mae = correctedSumAbs / static_cast<double>(sampleCount);
    state.maxAbs = correctedMaxAbs;
    return CALV3_OK;
}
                  ...
                  State& state)
{
    ...
    // Robustly initialize bL from the common part of all raw residuals.  The
    // subsequent analytic solve refines anchor XYZ and bL jointly.
    std::vector<double> rawResiduals(static_cast<size_t>(sampleCount));
    for (int s = 0; s < sampleCount; ++s)
    {
        const double distance = (state.points.col(s) - state.anchor).norm();
        if (!IsFinite(distance) || distance <= kTinyLength)
            return CALV3_ERR_INVALID_LENGTH;
        rawResiduals[static_cast<size_t>(s)] = measured(s) - distance;
        if (!IsFinite(rawResiduals[static_cast<size_t>(s)]))
            return CALV3_ERR_NONFINITE;
    }
    state.ropeBias = Median(rawResiduals);
    return EvaluateResiduals(measured, state);
}
                      ...
                      LinearSolveInfo& info)
{
    ...
    (void)measured;
    // The last four columns are nuisance variables: fixed-anchor XYZ and the
    // independent rope-chain bias bL.  The bL derivative is exactly +1 in
    // L_model=|P-lambda|+bL.  It is deliberately outside the 4N robot parameter
    // layout, so it cannot leak into MDH output or controller writeback.
...
{
    return "HE3-V3-CRAIG-MDH-ANALYTIC-20260826-R7.1-INDEPENDENT-ROPE-BIAS";
}
    ...
    CalibrationV3Report* report)
{
    ...
    report->maeBeforeM = before.maeBeforeZeroBias;
    report->maxAbsBeforeM = before.maxAbsBeforeZeroBias;
    report->ropeBiasM = before.ropeBias;
    report->maeBeforeZeroBiasM = before.maeBeforeZeroBias;
    report->maeBeforeGeometryM = before.mae;
    report->anchorRank = before.anchorRank;
    for (int i = 0; i < 3; ++i)
        report->anchorBefore[i] = before.anchor(i);
    for (int s = 0; s < sampleCount; ++s)
        residualBeforeM[s] = measured(s) - (before.points.col(s) - before.anchor).norm();
    ...
    for (int iter = 0; iter < maxIterations; ++iter)
    {
        ...
        if (requireFullColumnRank != 0 && jInfo.rank < requiredJointRank)
        {
            ...
            report->ropeBiasM = currentState.ropeBias;
            report->maeBeforeZeroBiasM = currentState.maeBeforeZeroBias;
            report->maeAfterM = currentState.mae;
            report->maeAfterGeometryM = currentState.mae;
        ...
        const double deltaRopeBias = deltaJoint(K + 3);
        ...
        for (double scale = 1.0; scale >= 1.0 / 1024.0; scale *= 0.5)
        {
            ...
            status = EvaluateStateAtAnchor(candidate, sampleCount, jointRad, measured,
                                           ...
                                           currentState.ropeBias + scale * deltaRopeBias,
                                           candidateState);
            if (status != CALV3_OK)
                continue;
    ...
    report->ropeBiasM = currentState.ropeBias;
    // Compare the final geometry/anchor with and without bL. This isolates the
    // fixed rope-chain offset instead of letting a biased initial anchor hide it.
    report->maeBeforeZeroBiasM = currentState.maeBeforeZeroBias;
    report->maeAfterGeometryM = currentState.mae;
    for (int i = 0; i < 3; ++i)
