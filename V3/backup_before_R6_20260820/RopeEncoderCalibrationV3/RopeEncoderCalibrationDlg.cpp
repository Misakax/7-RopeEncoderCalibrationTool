...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R5 2026.08.19");
//文件的路径
string m_FilePath;
//查询编码器值的指令
	...
	CalibrationV3Report* report)
{
	...
	// V1-port profile for the axial, freely rotating HE3 fixture:
	// d4, d5, terminal dN and q2..q5. q1 is an anchor gauge; q6/q7 are
	// not independently observable; terminal dN already contains ToolOffset.
	std::vector<int> active(4 * axis, 0);
	active[axis + 3] = 1;             // d4
	active[axis + 4] = 1;             // d5
	active[axis + axis - 1] = 1;      // terminal dN (d7 on HE3 seven-axis)
	const int lastObservableQ = (axis < 5) ? axis : 5;
	for (int joint = 1; joint < lastObservableQ; ++joint)
	{
...
void RopeEncoderCalibrationDlg::OnBnClickedButton7()
{
	...
	else 
	{
		...
		// R5 production gate: a low fitted residual alone is insufficient.
		...
		const bool lengthGate =
			std::fabs(m_CalibResult[3] - m_ThisRbtDH(1, 3)) <= 0.020 &&
			std::fabs(m_CalibResult[4] - m_ThisRbtDH(1, 4)) <= 0.020 &&
			std::fabs(m_CalibResult[5] - m_ThisRbtDH(1, m_RobotAxis - 1)) <= 0.020;
		bool zeroGate = true;
		const int zeroJointEnd = (m_RobotAxis < 5) ? m_RobotAxis : 5;
		for (int joint = 1; joint < zeroJointEnd; ++joint)
			zeroGate = zeroGate && std::fabs(m_CalibResult[6 + joint]) <= 5.0;
		m_R5CalibrationPassed = residualGate && conditionGate && repeatGate && fixtureGate &&
			point19Gate && lengthGate && zeroGate;

		CString gateLog;
		gateLog.Format(_T("R5 gate pass=%d residual=%d condition=%d repeat=%d fixture=%d point19=%d length=%d zero=%d max_sample_std_mm=%.6f j7_range_mm=%.6f point19_range_mm=%.6f\n"),
			m_R5CalibrationPassed ? 1 : 0, residualGate ? 1 : 0, conditionGate ? 1 : 0,
			repeatGate ? 1 : 0, fixtureGate ? 1 : 0, point19Gate ? 1 : 0,
			lengthGate ? 1 : 0, zeroGate ? 1 : 0, maxSampleStdMm,
	...
	{
		MessageBox(_T("R5计算已完成，但未通过量产安全门限。\r\n"
			"要求：MAE<=2mm、最大残差<=10mm、条件数<=1000、\r\n"
			"单点20次标准差<=0.5mm、J7扫描及19点重复极差<=1mm、\r\n"
			"杆长补偿<=20mm、零位补偿<=5度。\r\n"
			"结果仅供诊断，已禁止保存和写回机器人。"),
			_T("R5标定结果未通过"), MB_OK | MB_ICONWARNING);
	}
...
void RopeEncoderCalibrationDlg::OnBnClickedButton8()
{
	if (!m_R5CalibrationPassed)
	{
		MessageBox(_T("当前R5结果未通过安全门限，禁止保存或写回机器人。"),
			_T("R5写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	PowerOff();
	// TODO: 在此添加控件通知处理程序代码
