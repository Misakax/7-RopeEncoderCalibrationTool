...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R6.5 Closed Loop 2026.08.26");
//文件的路径
string m_FilePath;
//查询编码器值的指令
...
BOOL RopeEncoderCalibrationDlg::OnInitDialog()
{
	...
	AppendR6Monitor(_T("R6.5闭环监控已启动。右侧显示模型来源、开放参数、每点采集、每轮修正及写回回读；监控本身不控制机器人。"), false);
...
void RopeEncoderCalibrationDlg::OnBnClickedButton1()
{
	...
	SendInit();
	QKMLinkEventReset();
	// Read the controller before the keep-alive thread starts sending commands.
	// A validated readback becomes the next calibration input; a malformed
	// snapshot is logged and the local Craig-MDH configuration is retained.
	ReloadControllerModelAsInput(_T("连接成功后的控制器快照"), true);
	m_KeepConnect = true;
...
{
	CString text, line;
	line.Format(_T("R6.5 Craig MDH解析雅可比 + 激光开放策略 + 写回闭环\r\n轴数=%d 记录=%d\r\n"),
...
bool RopeEncoderCalibrationDlg::ReloadControllerModelAsInput(const CString& reason, bool updateInput)
{
	...
	const bool plausibleD = dMm[1] > 35.0 && dMm[1] < 65.0 &&
	...
	{
		CString rejected;
		rejected.Format(_T("控制器快照未通过HE3模型合理性检查（D=%d Alpha=%d Zero=%d），不作为算法输入。"),
...
void RopeEncoderCalibrationDlg::OnBnClickedButton7()
{
	...
	else 
	{
		...
		CString gateLog;
			gateLog.Format(_T("R6.5 gate pass=%d residual=%d condition=%d rank=%d motion=%d repeat=%d fixture=%d point19=%d observability=%d length=%d alpha=%d zero=%d laser_envelope=%d q6_active=%d max_target_error_deg=%.6f bad_point=%d max_sample_std_mm=%.6f j7_range_mm=%.6f point19_range_mm=%.6f q6_sensitivity_mm_per_deg=%.6f q7_sensitivity_mm_per_deg=%.9f q6_alpha6_cosine=%.6f\n"),
	...
	{
			MessageBox(_T("R6.5计算已完成，但未通过量产安全门限。\r\n"
			...
			"结果仅供诊断，已禁止保存和写回机器人。"),
				_T("R6.5标定结果未通过"), MB_OK | MB_ICONWARNING);
	}
...
void RopeEncoderCalibrationDlg::OnBnClickedR6Feedback()
{
	...
	{
		MessageBox(_T("当前没有可复输入的R6.5计算结果。"), _T("R6.5复输入检验"), MB_OK | MB_ICONINFORMATION);
		return;
	}
	...
	{
		MessageBox(_T("内存中的关节数据尺寸不完整，不能复输入检验。"), _T("R6.5复输入检验"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	for (int point = 0; point < m_TotalNum; ++point)
	{
		...
		if (value.IsEmpty())
		{
			MessageBox(_T("拉线点位数据不完整，不能复输入检验。"), _T("R6.5复输入检验"), MB_OK | MB_ICONERROR);
			return;
		}
	...
	MessageBox(summary, _T("R6.5复输入检验"), MB_OK | MB_ICONINFORMATION);
}
...
void RopeEncoderCalibrationDlg::OnBnClickedButton8()
{
	...
	CString saveInfo;
	saveInfo.Format(_T("准备R6.5完整写回：MAE %.3f mm，MAX %.3f mm，安全门限=%s。"),
	...
	if (!m_R5CalibrationPassed || !m_R6HaveResult || m_RobotAxis != 7)
	{
		...
		MessageBox(_T("当前结果不满足R6.5写回条件，未修改机器人。"),
			_T("R6.5写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	candidateLog.Format(_T("R6.5候选（写入前）\r\nD(mm): %s\r\n旧式Alpha(deg): %s\r\nZero(cnt): %s\r\n"),
	...
	if (!ValidateR65Candidate(candidateD, candidateAlpha, candidateZero, candidateFailure))
	{
		...
		MessageBox(candidateFailure, _T("R6.5写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	if (PowerOff() != 0)
	{
		AppendR6Monitor(_T("机器人下电命令未确认成功，禁止写参数。"));
		MessageBox(_T("机器人下电未确认，未执行写回。"), _T("R6.5写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (!writeOk)
	{
		...
		MessageBox(_T("写回或回读校验失败，已尝试恢复写入前快照，未持久化。请查看右侧日志。"),
			_T("R6.5写回失败"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (WriteIDNValue(m_IDNSave, m_IndexStr, m_IDNSaveOption) != 0)
	{
		...
		MessageBox(_T("参数逐项写入成功，但持久化保存失败。请勿继续测试，先检查控制器。"),
			_T("R6.5保存失败"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (!ReloadControllerModelAsInput(_T("持久化保存后回读"), true))
	{
		MessageBox(_T("保存命令成功，但保存后完整回读失败。不要开始下一轮，请查看右侧日志。"),
			_T("R6.5回读失败"), MB_OK | MB_ICONERROR);
		return;
	}

	_mkdir("./Result");
	const std::string stamp = m_FinishTime.empty() ? "manual" : m_FinishTime;
	const std::string path = "./Result/R65Writeback_" + stamp + ".csv";
	...
	AppendR6Monitor(_T("R6.5闭环完成：保存后的控制器回读已成为下一次原始MDH；请重新采集后计算真实复验精度。写回记录：") + CString(path.c_str()));
	m_SaveBtn.EnableWindow(FALSE);
	MessageBox(_T("保存、逐项回读和下一轮输入更新均成功。下一步必须重新采集，不能用旧拉线数据宣称复验精度。"),
		_T("R6.5闭环完成"), MB_OK | MB_ICONINFORMATION);
}
