...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R6 2026.08.20");
//文件的路径
string m_FilePath;
//查询编码器值的指令
...
BOOL RopeEncoderCalibrationDlg::OnInitDialog()
{
	...
	m_R6TraceEdit.Create(WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL |
		...
		this, IDC_R6_TRACE);
	m_R6TraceEdit.SetWindowTextW(_T("R6迭代记录将在计算后显示。\r\n"
		"结构d7与工具49mm分开解释；计算终端长度仍按二者之和。"));
	...
	CalibrationV3Report* report)
{
	...
	// R6 HE3 profile, aligned with the eight laser-calibration files:
	// d2..d5, alpha2..alpha6 and q2..q5.  The 93 mm structure + 49 mm
	// axial tool is mechanical and is not allowed to become a fictitious d7.
	// q1 is an anchor-frame gauge; q6/q7 are not independently observable
	// from this axial one-distance measurement.
...
{
	CString text, line;
	line.Format(_T("R6解析雅可比 + 激光样本物理约束\r\n轴数=%d 记录=%d\r\n"),
...
void RopeEncoderCalibrationDlg::OnBnClickedButton7()
{
	...
	else 
	{
		...
		// R6 production gate: a low fitted residual alone is insufficient.
		...
		bool zeroGate = true;
		const int zeroJointEnd = (m_RobotAxis < 5) ? m_RobotAxis : 5;
		for (int joint = 1; joint < zeroJointEnd; ++joint)
		zeroGate = zeroGate && std::fabs(m_CalibResult[6 + joint]) <= 2.01;
		m_R5CalibrationPassed = residualGate && conditionGate && rankGate && motionGate &&
			repeatGate && fixtureGate && point19Gate && lengthGate && alphaGate && zeroGate;

		CString gateLog;
		gateLog.Format(_T("R6 gate pass=%d residual=%d condition=%d rank=%d motion=%d repeat=%d fixture=%d point19=%d length=%d alpha=%d zero=%d max_target_error_deg=%.6f bad_point=%d max_sample_std_mm=%.6f j7_range_mm=%.6f point19_range_mm=%.6f\n"),
			...
			lengthGate ? 1 : 0, alphaGate ? 1 : 0, zeroGate ? 1 : 0,
...
void RopeEncoderCalibrationDlg::OnBnClickedR6Feedback()
{
	...
	{
		MessageBox(_T("当前没有可回灌的R6计算结果。"), _T("R6复算"), MB_OK | MB_ICONINFORMATION);
		return;
	}
	for (int joint = 0; joint < m_RobotAxis; ++joint)
	{
	...
	MessageBox(_T("已在内存中将本轮完整MDH作为下一轮输入。\r\n"
		"不会写机器人；现在复算同一批数据。"), _T("R6复算"), MB_OK | MB_ICONINFORMATION);
	OnBnClickedButton7();
}
...
void RopeEncoderCalibrationDlg::OnBnClickedR6Restore()
{
	if (!m_R6HaveOriginalDH)
	{
		MessageBox(_T("没有保存的原始MDH。"), _T("R6恢复"), MB_OK | MB_ICONINFORMATION);
		return;
	}
...
void RopeEncoderCalibrationDlg::OnBnClickedButton8()
{
	if (!m_R5CalibrationPassed)
	{
		MessageBox(_T("当前R6结果未通过安全门限，禁止保存或写回机器人。"),
			_T("R6写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	PowerOff();
	// TODO: 在此添加控件通知处理程序代码
