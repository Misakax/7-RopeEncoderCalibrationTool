...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R6.3 Craig MDH 2026.08.25");
//文件的路径
string m_FilePath;
//查询编码器值的指令
...
BOOL RopeEncoderCalibrationDlg::OnInitDialog()
{
	...
	// R6.3 restores the compact V1 window. Iteration details stay in the CSV/log;
	// only two small, non-motion diagnostic buttons remain in the result area.
	CRect beforeRect;
...
{
	CString text, line;
	line.Format(_T("R6.3 Craig MDH解析雅可比 + 激光8机先验 + q6可观测性诊断\r\n轴数=%d 记录=%d\r\n"),
...
void RopeEncoderCalibrationDlg::OnBnClickedR6Feedback()
{
	...
	{
		MessageBox(_T("当前没有可复输入的R6.3计算结果。"), _T("R6.3复输入检验"), MB_OK | MB_ICONINFORMATION);
		return;
	}

	// Preserve the first production result. The second solve is a same-data
	// convergence check only and must never silently replace the write candidate.
	const MatrixXd inputDh = m_ThisRbtDH;
	...
	double firstCalibResult[12];
	for (int i = 0; i < 12; ++i) firstCalibResult[i] = m_CalibResult[i];
	CString firstText[9];
	m_A1.GetWindowText(firstText[0]); m_A2.GetWindowText(firstText[1]);
	m_D1.GetWindowText(firstText[2]); m_D2.GetWindowText(firstText[3]);
	m_D3.GetWindowText(firstText[4]); m_D4.GetWindowText(firstText[5]);
	m_JointShow.GetWindowText(firstText[6]);
	m_ErrorBefore.GetWindowText(firstText[7]); m_ErrorAfter.GetWindowText(firstText[8]);
	...
	const CalibrationV3Report secondReport = m_R6LastReport;
	...
	for (int i = 0; i < 12; ++i) m_CalibResult[i] = firstCalibResult[i];
	m_A1.SetWindowText(firstText[0]); m_A2.SetWindowText(firstText[1]);
	m_D1.SetWindowText(firstText[2]); m_D2.SetWindowText(firstText[3]);
	m_D3.SetWindowText(firstText[4]); m_D4.SetWindowText(firstText[5]);
	m_JointShow.SetWindowText(firstText[6]);
	m_ErrorBefore.SetWindowText(firstText[7]); m_ErrorAfter.SetWindowText(firstText[8]);
	m_SaveBtn.EnableWindow(firstPassed ? TRUE : FALSE);
	m_R6FeedbackBtn.EnableWindow(TRUE);

	CString summary;
	summary.Format(_T("同一批数据复输入检验完成（不运动、不写机器人、不替换第一次结果）。\r\n\r\n"
		"第一次：MAE %.3f -> %.3f mm，最大残差 %.3f mm\r\n"
		"第二次：MAE %.3f -> %.3f mm，最大残差 %.3f mm\r\n\r\n"
		"第二次仅检查收敛趋势，不是独立精度验证；详细结果见 *_feedback2.csv。"),
		firstReport.maeBeforeM * 1000.0, firstReport.maeAfterM * 1000.0,
	...
	MessageBox(summary, _T("R6.3复输入检验"), MB_OK | MB_ICONINFORMATION);
}
...
void RopeEncoderCalibrationDlg::OnBnClickedButton8()
{
	...
	{
		MessageBox(_T("当前R6.3结果未通过安全门限，禁止保存或写回机器人。"),
			_T("R6.3写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	PowerOff();
	// TODO: 在此添加控件通知处理程序代码
