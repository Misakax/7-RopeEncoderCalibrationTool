
// RopeEncoderCalibration.cpp : 定义应用程序的类行为。
//

#include "stdafx.h"
#include "afxwinappex.h"
#include "afxdialogex.h"
#include "RopeEncoderCalibration.h"
#include "RopeEncoderCalibrationDlg.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#endif


// CRopeEncoderCalibrationApp

BEGIN_MESSAGE_MAP(CRopeEncoderCalibrationApp, CWinAppEx)

END_MESSAGE_MAP()


// CRopeEncoderCalibrationApp 构造

CRopeEncoderCalibrationApp::CRopeEncoderCalibrationApp()
{
	// 支持重新启动管理器
	m_dwRestartManagerSupportFlags = AFX_RESTART_MANAGER_SUPPORT_ALL_ASPECTS;
#ifdef _MANAGED
	// 如果应用程序是利用公共语言运行时支持(/clr)构建的，则: 
	//     1) 必须有此附加设置，“重新启动管理器”支持才能正常工作。
	//     2) 在您的项目中，您必须按照生成顺序向 System.Windows.Forms 添加引用。
	System::Windows::Forms::Application::SetUnhandledExceptionMode(System::Windows::Forms::UnhandledExceptionMode::ThrowException);
#endif

	// TODO: 将以下应用程序 ID 字符串替换为唯一的 ID 字符串；建议的字符串格式
	//为 CompanyName.ProductName.SubProduct.VersionInformation
	SetAppID(_T("RopeEncoderCalibration.AppID.NoVersion"));

	// TODO: 在此处添加构造代码，
	// 将所有重要的初始化放置在 InitInstance 中
}

// 唯一的一个 CRopeEncoderCalibrationApp 对象

CRopeEncoderCalibrationApp theApp;


// CRopeEncoderCalibrationApp 初始化

BOOL CRopeEncoderCalibrationApp::InitInstance()
{
	// 如果一个运行在 Windows XP 上的应用程序清单指定要
	// 使用 ComCtl32.dll 版本 6 或更高版本来启用可视化方式，
	//则需要 InitCommonControlsEx()。  否则，将无法创建窗口。
	INITCOMMONCONTROLSEX InitCtrls;
	InitCtrls.dwSize = sizeof(InitCtrls);
	// 将它设置为包括所有要在应用程序中使用的
	// 公共控件类。
	InitCtrls.dwICC = ICC_WIN95_CLASSES;
	InitCommonControlsEx(&InitCtrls);

	CWinAppEx::InitInstance();

	// Keep every relative runtime artifact (Config/log/data/Joint/Result)
	// beside the executable, regardless of whether the tool is launched by
	// double-clicking it or from Visual Studio.
	TCHAR modulePath[MAX_PATH] = { 0 };
	if (::GetModuleFileName(NULL, modulePath, MAX_PATH) > 0)
	{
		CString executableDirectory(modulePath);
		const int separator = executableDirectory.ReverseFind(_T('\\'));
		if (separator > 0)
		{
			executableDirectory = executableDirectory.Left(separator);
			::SetCurrentDirectory(executableDirectory);
		}
	}


	// 初始化 OLE 库
	if (!AfxOleInit())
	{
		AfxMessageBox(IDP_OLE_INIT_FAILED);
		return FALSE;
	}

	AfxEnableControlContainer();

	EnableTaskbarInteraction(FALSE);

	// 使用 RichEdit 控件需要 AfxInitRichEdit2()	
	// AfxInitRichEdit2();

	// 标准初始化
	// 如果未使用这些功能并希望减小
	// 最终可执行文件的大小，则应移除下列
	// 不需要的特定初始化例程
	// 更改用于存储设置的注册表项
	// TODO: 应适当修改该字符串，
	// 例如修改为公司或组织名
	SetRegistryKey(_T("应用程序向导生成的本地应用程序"));

	RopeEncoderCalibrationDlg dlg;
	m_pMainWnd = &dlg;
	dlg.DoModal();

	// This is a dialog-only application. Returning TRUE here starts an MFC
	// message loop with no main window after the dialog closes, leaving a
	// seemingly "closed" but still-running background process.
	return FALSE;
}

int CRopeEncoderCalibrationApp::ExitInstance()
{
	//TODO: 处理可能已添加的附加资源
	AfxOleTerm(FALSE);

	return CWinAppEx::ExitInstance();
}

// CRopeEncoderCalibrationApp 消息处理程序


// 用于应用程序“关于”菜单项的 CAboutDlg 对话框

class CAboutDlg : public CDialogEx
{
public:
	CAboutDlg();

// 对话框数据
#ifdef AFX_DESIGN_TIME
	enum { IDD = IDD_ABOUTBOX };
#endif

protected:
	virtual void DoDataExchange(CDataExchange* pDX);    // DDX/DDV 支持

// 实现
protected:
	DECLARE_MESSAGE_MAP()
};

CAboutDlg::CAboutDlg() : CDialogEx(IDD_ABOUTBOX)
{
}

void CAboutDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CAboutDlg, CDialogEx)
END_MESSAGE_MAP()

// 用于运行对话框的应用程序命令
void CRopeEncoderCalibrationApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}

// CRopeEncoderCalibrationApp 消息处理程序



