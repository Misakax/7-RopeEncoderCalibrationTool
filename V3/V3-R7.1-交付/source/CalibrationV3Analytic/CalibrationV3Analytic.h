...
struct CalibrationV3Report
{
    ...
    // R7.1 rope-measurement-chain diagnostics.  ropeBiasM is the independently
    // identified constant bL in L_measured = |P-lambda| + bL.  It is never
    // included in outDelta4N and therefore can never be written as robot MDH.
    double ropeBiasM;
    double maeBeforeZeroBiasM;
    double maeBeforeGeometryM;
    double maeAfterGeometryM;
};
...
//    adjacent-sphere equations, then solved jointly with three anchor nuisance
//    variables and one independent rope-chain bias bL. bL is diagnostic only:
//    it is not present in outDelta4N and is never robot MDH. No finite-difference
//    Jacobian is used by the DLL.
