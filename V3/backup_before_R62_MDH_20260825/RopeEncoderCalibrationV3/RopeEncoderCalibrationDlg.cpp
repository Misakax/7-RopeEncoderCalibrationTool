...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R6.1 2026.08.21");
//文件的路径
string m_FilePath;
//查询编码器值的指令
...
BOOL RopeEncoderCalibrationDlg::OnInitDialog()
{
	...
	m_R6TraceEdit.Create(WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL |
		...
		this, IDC_R6_TRACE);
	m_R6TraceEdit.SetWindowTextW(_T("R6.1迭代记录将在计算后显示。\r\n"
	...
	CalibrationV3Report* report)
{
	...
	// R6.1 HE3 profile, aligned with the eight laser-calibration files:
	// d2..d5, alpha2..alpha6 and q2..q5 are normally visible. q6 is first
	...
	for (int joint = 1; joint <= 5 && joint < axis; ++joint)
		active[2 * axis + joint] = 1;   // alpha2..alpha6
	for (int joint = 1; joint <= 4 && joint < axis; ++joint)
	...
	// R6.1 observability evidence from the exact analytic point Jacobian.
	// We project alpha6/q6/q7 point derivatives onto the rope direction at
	...
	if (axis >= 7 && calibrationSampleCount > 0)
	{
		...
		for (int calibrationSample = 0; calibrationSample < calibrationSampleCount; ++calibrationSample)
		{
			...
			const int alpha6Column = 2 * axis + 5;
...
{
	CString text, line;
	line.Format(_T("R6.1解析雅可比 + q6可观测性诊断\r\n轴数=%d 记录=%d\r\n"),
...
void RopeEncoderCalibrationDlg::OnBnClickedButton7()
{
	...
	else 
	{
		...
		// R6.1 production gate: a low fitted residual alone is insufficient.
		...
		// R6.1 does not force a structurally zero joint into the solve.  With the
		...
		CString gateLog;
		gateLog.Format(_T("R6.1 gate pass=%d residual=%d condition=%d rank=%d motion=%d repeat=%d fixture=%d point19=%d observability=%d length=%d alpha=%d zero=%d q6_active=%d max_target_error_deg=%.6f bad_point=%d max_sample_std_mm=%.6f j7_range_mm=%.6f point19_range_mm=%.6f q6_sensitivity_mm_per_deg=%.6f q7_sensitivity_mm_per_deg=%.9f q6_alpha6_cosine=%.6f\n"),
	...
	{
		MessageBox(_T("R6.1计算已完成，但未通过量产安全门限。\r\n"
			...
			"结果仅供诊断，已禁止保存和写回机器人。"),
			_T("R6.1标定结果未通过"), MB_OK | MB_ICONWARNING);
	}
...
void RopeEncoderCalibrationDlg::OnBnClickedR6Feedback()
{
	...
	{
		MessageBox(_T("当前没有可回灌的R6.1计算结果。"), _T("R6.1复算"), MB_OK | MB_ICONINFORMATION);
		return;
	}
	for (int joint = 0; joint < m_RobotAxis; ++joint)
	{
	...
	MessageBox(_T("已在内存中将本轮完整MDH作为下一轮输入。\r\n"
		"不会写机器人；现在复算同一批数据。"), _T("R6.1复算"), MB_OK | MB_ICONINFORMATION);
	OnBnClickedButton7();
}
...
void RopeEncoderCalibrationDlg::OnBnClickedR6Restore()
{
	if (!m_R6HaveOriginalDH)
	{
		MessageBox(_T("没有保存的原始MDH。"), _T("R6.1恢复"), MB_OK | MB_ICONINFORMATION);
		return;
	}
...
void RopeEncoderCalibrationDlg::OnBnClickedButton8()
{
	if (!m_R5CalibrationPassed)
	{
		MessageBox(_T("当前R6.1结果未通过安全门限，禁止保存或写回机器人。"),
			_T("R6.1写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	PowerOff();
	// TODO: 在此添加控件通知处理程序代码
