...
int main()
{
    ...
    // R7.1 regression: inject a constant error in the rope measurement chain.
    // The solver must recover bL without inventing a d5/MDH correction.
    constexpr double kInjectedRopeBiasM = 0.0025;
    ...
    for (int s = 0; s < kSamples; ++s)
    {
        ...
        ropeRawBias[s] = std::sqrt(dx * dx + dy * dy + dz * dz)
            - ropeOffset + kInjectedRopeBiasM;
    }
    int activeBias[4 * kAxis] = {};
    activeBias[kAxis + 4] = 1; // observable sentinel; true MDH correction is zero
    ...
    const double fictitiousD5Mm = deltaBias[kAxis + 4] * 1000.0;
    const bool biasSuccessStatus = statusBias == CALV3_OK || statusBias == CALV3_STOP_NO_IMPROVEMENT;
    const bool passBias = biasSuccessStatus && reportBias.maeAfterGeometryM < 1e-7 &&
        std::abs(recoveredBiasMm - kInjectedRopeBiasM * 1000.0) < 0.01 &&
        std::abs(fictitiousD5Mm) < 0.01;
    std::cout << "rope_bias status=" << statusBias
              << " bL expected/recovered(mm): " << kInjectedRopeBiasM * 1000.0
              << " / " << recoveredBiasMm << "\n"
              << "raw/geometry MAE(mm): " << reportBias.maeBeforeZeroBiasM * 1000.0
              << " -> " << reportBias.maeAfterGeometryM * 1000.0
              << " fictitious_d5(mm)=" << fictitiousD5Mm << "\n"
              << "rope_bias_test=" << (passBias ? "PASS" : "FAIL") << "\n";

    const bool pass = passD5 && passQ5 && passQ6StructuralZero && passQ7StructuralZero && passBias;
