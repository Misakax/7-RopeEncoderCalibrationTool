...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R6.4 Monitor 2026.08.26");
//文件的路径
string m_FilePath;
//查询编码器值的指令
...
BOOL RopeEncoderCalibrationDlg::OnInitDialog()
{
	...
	// R6.4 keeps the complete V1 operation area and adds a read-only monitor on
	...
	AppendR6Monitor(_T("R6.4运行监控已启动。右侧仅显示信息，不控制机器人。"), false);
...
{
	CString text, line;
	line.Format(_T("R6.4 Craig MDH解析雅可比 + 激光8机先验 + q6可观测性诊断\r\n轴数=%d 记录=%d\r\n"),
...
{
	CString text, line;
	line.Format(_T("=== %s：当前计算输入（不是尚未保存的候选结果） ===\r\n"), operation.GetString());
	text += line;
...
void RopeEncoderCalibrationDlg::OnBnClickedButton7()
{
	...
	else 
	{
		...
		CString gateLog;
		gateLog.Format(_T("R6.3 gate pass=%d residual=%d condition=%d rank=%d motion=%d repeat=%d fixture=%d point19=%d observability=%d length=%d alpha=%d zero=%d q6_active=%d max_target_error_deg=%.6f bad_point=%d max_sample_std_mm=%.6f j7_range_mm=%.6f point19_range_mm=%.6f q6_sensitivity_mm_per_deg=%.6f q7_sensitivity_mm_per_deg=%.9f q6_alpha6_cosine=%.6f\n"),
	...
	{
		MessageBox(_T("R6.3计算已完成，但未通过量产安全门限。\r\n"
			...
			"结果仅供诊断，已禁止保存和写回机器人。"),
			_T("R6.3标定结果未通过"), MB_OK | MB_ICONWARNING);
	}
...
void RopeEncoderCalibrationDlg::OnBnClickedR6Feedback()
{
	...
	{
		MessageBox(_T("当前没有可复输入的R6.4计算结果。"), _T("R6.4复输入检验"), MB_OK | MB_ICONINFORMATION);
		return;
	}

	// R6.4 performs the second solve directly. R6.3 recursively invoked the
	...
	{
		MessageBox(_T("内存中的关节数据尺寸不完整，不能复输入检验。"), _T("R6.4复输入检验"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	for (int point = 0; point < m_TotalNum; ++point)
	{
		...
		if (value.IsEmpty())
		{
			MessageBox(_T("拉线点位数据不完整，不能复输入检验。"), _T("R6.4复输入检验"), MB_OK | MB_ICONERROR);
			return;
		}
	...
	MessageBox(summary, _T("R6.4复输入检验"), MB_OK | MB_ICONINFORMATION);
}
...
{
	ShowCurrentMdh(_T("保存操作前：控制器当前输入"));
	CString saveInfo;
	saveInfo.Format(_T("准备保存第一次候选结果：MAE %.3f mm，MAX %.3f mm，安全门限=%s。"),
	...
	AppendR6Monitor(_T("写回范围提醒：当前控制器接口沿用V1的12槽，只写a3、a4、d1、d4、d5、末端d及q1~q6。"
		"R6.4内部完整候选中的d2、d3、alpha2~alpha6没有对应的已确认写回IDN；"
		"因此本次保存是兼容性部分写回，不能把0.9 mm完整拟合结果直接视为已全部进入控制器。"));
	if (!m_R5CalibrationPassed)
	{
		MessageBox(_T("当前R6.4结果未通过安全门限，禁止保存或写回机器人。"),
			_T("R6.4写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	PowerOff();
	// TODO: 在此添加控件通知处理程序代码
	...
	if (bStt == 0) {
		AppendR6Monitor(_T("控制器保存命令返回成功。下一次实机验证前必须重新读取控制器/配置输入并核对右侧MDH；"
			"若要完整写回15参数，需先由控制器负责人确认d2、d3、alpha2~alpha6的IDN映射。"));
		MessageBox(_T("保存成功"), _T("提示"), MB_OK);
	}
	else
	{
		CString failInfo;
		failInfo.Format(_T("控制器保存命令失败，返回值=%d。"), bStt);
		AppendR6Monitor(failInfo);
	}
...
afx_msg LRESULT RopeEncoderCalibrationDlg::OnDisplayChange(WPARAM wParam, LPARAM lParam)
{
	...
	pointInfo.Format(_T("点位 %d/%d：拉线=%.6f mm，%d次标准差=%.6f mm；实际关节=%s"),
		static_cast<int>(wParam) + 1, m_TotalNum, EncoderValue, m_R5SamplesPerPose,
		stdMm, actual.GetString());
