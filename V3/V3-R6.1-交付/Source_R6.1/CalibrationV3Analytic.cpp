...
{
    // The prior is deliberately weak: it stabilizes poorly observable
    // combinations but must not bias a clean, full-rank measurement.  The
    // tighter production protection is supplied by R6HardLimit() and by the
    // GUI write gate below.
    if (fullIndex < 2 * axis) return 0.100;                  // a,d: 100 mm very weak prior
    return 30.0 * 3.14159265358979323846 / 180.0;           // alpha,q0: 30 degree very weak prior
}

double R6HardLimit(int fullIndex, int axis)
{
    if (fullIndex < 2 * axis) return 0.006;                  // a,d: 6 mm absolute correction
...
{
    return "HE3-V3-V1-ANALYTIC-20260821-R6.1";
}
