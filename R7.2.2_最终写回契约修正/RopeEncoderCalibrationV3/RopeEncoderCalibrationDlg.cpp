...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R7.2.2 Two-Stage + Laser Prior + Final Writeback Contract 2026.08.26");
//文件的路径
string m_FilePath;
//查询编码器值的指令
...
bool RopeEncoderCalibrationDlg::ReloadControllerModelAsInput(const CString& reason, bool updateInput)
{
	...
	if (legacyDuplicateD6)
	{
		...
		if (updateInput)
		{
			AppendR6Monitor(_T("拒绝把D6重复快照转成算法输入；R7.2.2继续固定使用公共HE3_GY-cfg.txt。"));
			return false;
		}
	}
...
bool RopeEncoderCalibrationDlg::PrepareR71PublicInputBeforeStart()
{
	...
	if (MessageBox(confirmation, _T("R7.2.2：确认公共MDH输入"),
		MB_OKCANCEL | MB_ICONWARNING) != IDOK)
		return false;

	AppendR6Monitor(_T("R7.2.2公共MDH输入已确认，允许开始本次运动采集。"));
	return true;
}
	...
	double value, double tolerance, CString& failure)
{
	...
	const std::string raw = ReadIDNValue(idn, m_IndexStr, index);
	const double verifyTolerance = integerIdn ? 0.01 : std::max(tolerance, 0.00051);
...
void RopeEncoderCalibrationDlg::OnBnClickedButton8()
{
	...
	CString saveInfo;
	saveInfo.Format(_T("准备R7.2.2写回：拉线标定仅允许写D2~D5、Alpha1~Alpha6、q2~q6；D6不是标定参数，D7永不写。去零偏后几何MAE %.3f mm，MAX %.3f mm，bL=%+.3f mm（仅测量链，不写MDH），安全门限=%s。"),
	...
	AppendR6Monitor(_T("R7.2.2最终闭环通过：拉线标定字段已持久化；D6按独立结构规则处理；D7保持写回前值且全过程未发送D7写命令。"));

	_mkdir("./Result");
	const std::string stamp = m_FinishTime.empty() ? "manual" : m_FinishTime;
	const std::string path = "./Result/R722Writeback_" + stamp + ".csv";
	...
	AppendR6Monitor(_T("R7.2.2闭环完成：保存后的控制器回读仅作为闭环证据；下一次点‘开始’会再次强制恢复公共HE3_GY-cfg.txt作为算法输入。写回记录：") + CString(path.c_str()));
	m_SaveBtn.EnableWindow(FALSE);
	MessageBox(
		_T("保存、逐项回读和最终闭环检查全部成功。D6若被修复，是独立HE3结构修复，不是拉线标定结果；D7未写。bL未写入MDH。下一轮仍从公共HE3_GY-cfg.txt重新采集。"),
		_T("R7.2.2闭环完成"), MB_OK | MB_ICONINFORMATION);
}
