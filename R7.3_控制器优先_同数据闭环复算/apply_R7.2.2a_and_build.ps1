...
Write-Host "=== R7.2.2a compile hotfix apply and build ===" -ForegroundColor Cyan
...
Require-Path $srcDll "R7.2.2a DLL source"
Require-Path $srcGui "R7.2.2a GUI source"
Require-Path $dstDll "target DLL source"
Require-Path $dstGui "target GUI source"

$srcGuiText = Get-Content -LiteralPath $srcGui -Raw
$srcDllText = Get-Content -LiteralPath $srcDll -Raw
if ($srcGuiText -notmatch "R7\.2\.2a") { throw "GUI identity check failed." }
if ($srcDllText -notmatch "R7\.2\.2a") { throw "DLL identity check failed." }

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backup = Join-Path $BuildRoot ("backup_before_R7.2.2a_" + $stamp)
...
Write-Host "R7.2.2a source applied." -ForegroundColor Green
...
Write-Host "EXE: $exe"
Write-Host "DLL: $dll"
Write-Host "Expected GUI version: R7.2.2a" -ForegroundColor Yellow
