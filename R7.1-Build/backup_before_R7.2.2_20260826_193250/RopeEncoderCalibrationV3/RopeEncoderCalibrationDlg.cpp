...
#pragma region 全局变量
//软件版本
CString m_Version = _T(" HE3 V3 Analytic R7.2 Two-Stage + Laser Prior 2026.08.26");
//文件的路径
string m_FilePath;
//查询编码器值的指令
...
BOOL RopeEncoderCalibrationDlg::OnInitDialog()
{
	...
	AppendR6Monitor(_T("R7.2闭环监控已启动。算法输入固定为公共 Config/RobotType/HE3_GY-cfg.txt；阶段1固定机器人模型求拉线零偏bL，阶段2冻结固定端与bL，并在8台激光统计先验/3σ包络内辨识MDH；bL不写入MDH。"), false);
...
void RopeEncoderCalibrationDlg::OnBnClickedButton2()
{
	...
	{
		AppendR6Monitor(_T("R7.2本次采集已取消：公共HE3_GY-cfg.txt输入未通过准备检查。"));
		return;
	}
...
void RopeEncoderCalibrationDlg::ShowR65ModelContract(const CString& operation)
{
	...
	text += _T("R7.2输入固定：公共 Config/RobotType/HE3_GY-cfg.txt；不使用控制器回读MDH作为算法初值。\r\n");
...
bool RopeEncoderCalibrationDlg::ReloadControllerModelAsInput(const CString& reason, bool updateInput)
{
	...
	{
		CString rejected;
		rejected.Format(_T("控制器快照未通过HE3模型合理性检查（D=%d Alpha=%d Zero=%d），不作为算法输入。R7.2要求D1/D6接近0 mm、D7为末端结构长度；可防止旧六轴写回把93 mm误写到D6。"),
	...
	{
		AppendR6Monitor(_T("检测到已知旧版D6/D7重复末端长度：D6与D7均约93 mm。8台激光Robot1.qxml均为D6=0、D7=93。该快照仅用于写回前备份；R7.2算法仍固定公共MDH。"));
		if (updateInput)
		{
			AppendR6Monitor(_T("拒绝把D6重复快照转成算法输入；请继续使用R7.2公共MDH。"));
			return false;
		}
	}
...
bool RopeEncoderCalibrationDlg::PrepareR71PublicInputBeforeStart()
{
	if (m_RobotAxis != 7)
	{
		AppendR6Monitor(_T("R7.2公共MDH固定输入仅对HE3七轴启用；当前按已加载配置继续。"));
		return true;
	}
	...
	{
		AppendR6Monitor(_T("R7.2公共MDH不可用：Config/RobotType/HE3_GY-cfg.txt启动快照无效，禁止运动。"));
		MessageBox(_T("公共 HE3_GY-cfg.txt 的启动快照无效，本次不会运动机器人。"),
			_T("R7.2：公共MDH不可用"), MB_OK | MB_ICONERROR);
		return false;
	}

	// R7.2 contract: every new acquisition starts from the immutable public
	...
	m_R65InputSource = m_R7OriginalConfigSource + _T("（R7.2固定公共输入）");
	m_R6HaveOriginalDH = false;
	m_R6HaveResult = false;
	m_R6FeedbackBtn.EnableWindow(FALSE);
	m_SaveBtn.EnableWindow(FALSE);

	AppendR6Monitor(_T("R7.2已强制恢复公共HE3_GY-cfg.txt Craig MDH；控制器快照不会作为本次算法输入。"));
	ShowR65ModelContract(_T("R7.2固定公共MDH输入"));
	ShowCurrentMdh(_T("运动前公共MDH输入"));

	CString confirmation;
	confirmation.Format(_T("R7.2本次算法输入固定为：%s\r\n\r\n"
		"完整MDH已写入右侧日志。\r\n"
		"按‘确定’后开始运动；按‘取消’则不运动。"),
		m_R65InputSource.GetString());
	if (MessageBox(confirmation, _T("R7.2：确认公共MDH输入"),
		MB_OKCANCEL | MB_ICONWARNING) != IDOK)
		return false;

	AppendR6Monitor(_T("R7.2公共MDH输入已确认，允许开始本次运动采集。"));
	return true;
}
...
void RopeEncoderCalibrationDlg::OnBnClickedButton8()
{
	...
	CString saveInfo;
	saveInfo.Format(_T("准备R7.2完整写回：去零偏后几何MAE %.3f mm，MAX %.3f mm，bL=%+.3f mm（仅测量链，不写MDH），安全门限=%s。"),
	...
	if (!m_R5CalibrationPassed || !m_R6HaveResult || m_RobotAxis != 7)
	{
		...
		MessageBox(_T("当前结果不满足R7.2写回条件，未修改机器人。"),
			_T("R7.2写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	const bool repairLegacyD6 = beforeD.size() == 7 && beforeD[5] > 75.0 && beforeD[5] < 110.0 &&
		beforeD[6] > 75.0 && beforeD[6] < 110.0 && std::fabs(beforeD[5] - beforeD[6]) <= 2.0;
	if (repairLegacyD6)
		candidateD[5] = 0.0; // restore the HE3 structure proven by all eight laser Robot1.qxml files

	// Laser scheme: D2..D5, physical Alpha1..Alpha6 and q2..q6.  A, D1/D6/D7,
	// q1/q7 and Alpha7 stay exactly at the controller snapshot. D6 also stays
	// untouched unless the exact known legacy D6=D7~93 duplicate is detected;
	// in that case candidate D6 is explicitly repaired to 0 mm after confirmation. Craig alpha
	...
	candidateLog.Format(_T("R7.2候选（写入前；bL不在候选中）\r\nD(mm): %s\r\n旧式Alpha(deg): %s\r\nZero(cnt): %s\r\n"),
	...
	if (!ValidateR65Candidate(candidateD, candidateAlpha, candidateZero, candidateFailure))
	{
		...
		MessageBox(candidateFailure, _T("R7.2写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	if (repairLegacyD6)
	{
		if (MessageBox(_T("检测到控制器当前D6与D7都约为93 mm。已上传的8台HE3激光校准Robot1.qxml全部为D6=0、D7=93。\r\n\r\n本次保存除D2~D5/Alpha/零位外，还将修复D6为0 mm；D7保持原93 mm。是否继续？"),
			_T("R7.2：确认修复旧版D6重复值"), MB_YESNO | MB_ICONWARNING) != IDYES)
		{
			AppendR6Monitor(_T("用户取消D6结构修复，本次未写回任何参数。"));
			return;
		}
	}
	...
	if (PowerOff() != 0)
	{
		...
		MessageBox(_T("机器人下电未确认，未执行写回。"), _T("R7.2写回保护"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (repairLegacyD6 && writeOk)
		writeOk = WriteControllerValueVerified(m_IDND, 6, candidateD[5], 0.0005, failure);
	...
	if (!writeOk)
	{
		...
		MessageBox(_T("写回或回读校验失败，已尝试恢复写入前快照，未持久化。请查看右侧日志。"),
			_T("R7.2写回失败"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (WriteIDNValue(m_IDNSave, m_IndexStr, m_IDNSaveOption) != 0)
	{
		...
		MessageBox(_T("参数逐项写入成功，但持久化保存失败。请勿继续测试，先检查控制器。"),
			_T("R7.2保存失败"), MB_OK | MB_ICONERROR);
		return;
	}
	...
	if (!ReloadControllerModelAsInput(_T("持久化保存后回读"), false))
	{
		MessageBox(_T("保存命令成功，但保存后完整回读失败。不要开始下一轮，请查看右侧日志。"),
			_T("R7.2回读失败"), MB_OK | MB_ICONERROR);
		return;
	}

	_mkdir("./Result");
	const std::string stamp = m_FinishTime.empty() ? "manual" : m_FinishTime;
	const std::string path = "./Result/R72Writeback_" + stamp + ".csv";
	...
	if (out)
	{
		...
		out << "legacy_d6_duplicate_repaired," << (repairLegacyD6 ? 1 : 0) << "\n";
	...
	AppendR6Monitor(_T("R7.2闭环完成：保存后的控制器回读仅作为闭环证据；下一次点‘开始’会再次强制恢复公共HE3_GY-cfg.txt作为算法输入。写回记录：") + CString(path.c_str()));
	m_SaveBtn.EnableWindow(FALSE);
	MessageBox(_T("保存和逐项回读成功；bL未写入MDH。下一轮开始时仍会使用公共HE3_GY-cfg.txt，并且必须重新采集，不能用旧拉线数据宣称复验精度。"),
		_T("R7.2闭环完成"), MB_OK | MB_ICONINFORMATION);
}
