...
void ParaSplit(const string& s, double* v) {
...
void DecodePara(string tMsg, RbtConfig &tPara) {
	...
	if (tIdx > 0) {
		...
		case a:
			...
			ParaSplit(tempValue, tPara.a);
			break;
		case alpha:
			tPara.alpha = new double[tPara.RobotAxis];
			ParaSplit(tempValue, tPara.alpha);
		...
		case d:
			...
			ParaSplit(tempValue, tPara.d);
			break;
		case theta:
			tPara.theta = new double[tPara.RobotAxis];
			ParaSplit(tempValue, tPara.theta);
			for (size_t i = 0; i < tPara.RobotAxis; i++)
			{
		...
		case beta:
			...
			ParaSplit(tempValue, tPara.beta);
			for (size_t i = 0; i < tPara.RobotAxis; i++)
			{
				tPara.beta[i] *= PI / 180;
			}
		...
		case EncoderLineNum:
			...
			ParaSplit(tempValue, tPara.EncoderLineNum);
			break;
		case MoveDirection:
			tPara.MoveDirection = new double[tPara.RobotAxis];
			ParaSplit(tempValue, tPara.MoveDirection);
			break;
		case MotionWaittime:
...
void ReadConfigFile(string tPath, RbtConfig& Paras) {
